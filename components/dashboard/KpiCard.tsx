...
// 暖色パレット: 旧カラー名をキーとして維持しつつ、配色を全て暖色に置換
const COLOR_MAP = {
  indigo: {
  ...
  violet: {
    ...
    accent: "from-red-500/20 via-red-500/0 to-transparent",
    ...
    <div className="card relative overflow-hidden p-5 group hover:shadow-md transition-shadow duration-200">
      ...
      <div className="relative">
        ...
        <div className="flex items-center gap-2 mt-3 h-4">
          {trend && (
            <span
              className={`inline-flex items-center gap-0.5 text-xs font-semibold ${
                trend.positive ? "text-amber-600" : "text-red-500"
              }`}
            >
          ...
          {subtitle && (
            <p className="text-[11px] text-stone-400 font-medium truncate">{subtitle}</p>
          )}
        </div>
      </div>
    </div>
