...
const MEDAL_COLORS = [
  ...
  "bg-gradient-to-br from-orange-400 to-amber-600 text-white shadow-sm shadow-orange-500/40",
];
              ...
              >
                <td className="py-3 pr-3">
                  {i < 3 ? (
                    ...
                    <span className="inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold text-slate-400 bg-slate-100">
                      {i + 1}
                    </span>
                  )}
                </td>
                <td className="py-3 pr-3">
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">{store.prefecture}</p>
                </td>
                <td className="py-3 pr-3 text-right text-slate-700 font-medium tabular-nums">
                  {formatJPY(metrics.spend)}
                </td>
                <td className="py-3 pr-3 text-right text-slate-700 tabular-nums">
                  {Math.round(metrics.leads)}
                </td>
                <td className="py-3 pr-3 text-right text-slate-700 tabular-nums">
                  {Math.round(metrics.contracts)}
                </td>
                <td className="py-3 pr-3 text-right text-slate-900 font-semibold tabular-nums">
